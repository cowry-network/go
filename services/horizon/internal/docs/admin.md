...
2. Clone the Stellar Go monorepo:  `go get github.com/cowry-network/go`. You should see the repository cloned at `$GOPATH/src/github.com/cowry-network/go`.
...
4. Compile the Horizon binary: `cd $GOPATH; go install github.com/cowry-network/go/services/horizon`. You should see the `horizon` binary in `$GOPATH/bin`.
