...
then
    ...
    git push --delete -q https://$GITHUB_OAUTH_TOKEN@github.com/cowry-network/go snapshots || true
    git tag snapshots -a -m "Generated snapshots from TravisCI for build $TRAVIS_BUILD_NUMBER on $(date --utc +'%F-%T')"
    git push -q https://$GITHUB_OAUTH_TOKEN@github.com/cowry-network/go --tags
fi
